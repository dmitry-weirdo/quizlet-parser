...
**Карточка:** `Кока-Кола` → `Какой бренд должен щёлкать на Атланту?`
...
- `entities` — Те сущности, которые можно выделить из заданного текста. Эти сущности, как правило, и нужно использовать как ответ в карточках.
